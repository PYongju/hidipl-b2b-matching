...
class AzureOpenAIExplanationProvider(ExplanationProvider):
    ...
    def _system_prompt(self) -> str:
        return (
            ...
            "값이 비어있더나 null인 경우 반드시 weaknesses로 명시해주세요\n"
            "같은 응답의 다른 후보에 동일 업체가 있어도 이 아이템에 그 이름을 쓰지 마세요. "
            "반드시 vendor_name 을 '업체 미확인' 으로만 표시하세요.\n"
            "- 같은 영역(가격/납기/보증/사양/설치)에 대한 여러 사실은 한 줄에 묶어 하나의 weaknesses 로 작성하세요. "
            "-예: '가격 경쟁력 낮고 최저가 대비 5% 초과' → 하나의 weaknesses. "
            ...
            "이 weaknesses를 다른 weaknesses 보다 우선 작성하세요.\n"
            "rank(순위)와 rinal_score(종합점수)가 일관되면 weaknesses와 strengths에 모두 명시하지 않습니다.\n"
            ...
            "- overall_summary는 weaknesses와 strengths 기반 주요 항목 위주로 작성하세요.\n"
            ...
            "- filter_reasons가 존재하면 반드시 사유를 weaknesses로 기재하세요"
            "- JSON만 반환하세요.\n"
            "출력 스키마: "
