...
export default function ProjectRequirementsPage({
  ...
  const { statusMessage, startSaving, markSaved } = useAutoSaveStatus();
  ...
  useEffect(() => {
    ...
    autoSaveTimerRef.current = setTimeout(() => {
      Promise.resolve(onAutoSave(buildProjectRequestPayload(projectData))).finally(
        () => {
          markSaved();
        },
      );
  ...
  useEffect(() => {
    ...
    if (onAutoSave && projectData.projectApiId) return undefined;
    ...
    <div className="flow-page requirements-page">
      ...
      <main className="requirements-main">
        ...
        <section className="requirements-layout">
            ...
            <section className="requirement-input-block">
              <div className="requirement-block-title">
                ...
                <div>
                  <b>추가 메모</b>
                  <small>
                    A/S, 납기, 설치 제한, 첨부 자료에 대한 메모를 남겨요.
                  </small>
                </div>
              </div>
              <label>
              ...
              <label>
                ...
                <textarea
                  onChange={(event) =>
                    updateField("attachmentMemo", event.target.value)
                  }
          ...
          <aside className="matching-check-panel refined sticky-panel">
            <div className="requirements-section-title">
              <div>
                <h2>매칭 가능성 체크</h2>
            ...
            <div className="matching-next-guide refined">
              <b>다음 단계 안내</b>
              <p>
                파트너 매칭 화면에서 AI 추천 순위와 적합도 점수를 확인하고,
                견적 요청을 보낼 공급사를 선택할 수 있어요.
              </p>
            </div>
          </aside>
        </section>
      ...
      <FlowBottomBar
        backLabel="이전 단계"
        nextDisabled={isPartnerMatchingLoading}
        nextLabel={
...
function getMatchingChecks(data) {
  ...
  const requiredFilledCount = [hasCompany, hasLocation, hasUsage].filter(
    Boolean,
  ).length;
  ...
  return [
    {
      ...
      message:
        requiredFilledCount === 3
          ? "회사명, 설치 위치, 활용 용도 및 디스플레이 요구사항이 모두 입력됐어요."
          : `필수 입력 3개 중 ${requiredFilledCount}개가 입력됐어요.`,
    },
    {
      ...
      message: stage.includes("정보 탐색")
        ? "정보 탐색 단계에서는 자동 매칭 결과가 제한될 수 있어요."
...
function getFieldReadinessScore(data) {
  ...
  const items = [
    { filled: hasCompany, weight: 20 },
    { filled: hasLocation, weight: 20 },
    { filled: hasUsage, weight: 20 },
    { filled: hasProjectName, weight: 8 },
    { filled: hasProjectDate, weight: 8 },
    { filled: hasDisplay, weight: 10 },
    { filled: hasQuantity, weight: 4 },
    { filled: hasBudget, weight: 4 },
    { filled: hasSolutions, weight: 3 },
    { filled: hasReviewPreset, weight: 3 },
  ];
...
function getReadinessMessage(readiness, checks) {
  ...
  return "필수 정보부터 입력하면 매칭 준비도가 올라가요.";
}
